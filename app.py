...
translations = {
    "en": {
        ...
        "support": "Support",
        "footer": "Digital Governance for Uganda | Beta Version",
...
if not os.path.exists(SECTORS_FILE):
    default_sectors = {
        "agriculture": {"name": "🌾 الزراعة", "fields": ["farmer_name", "farmer_phone", "village", "crop_type", "quantity", "price"]},
        "health": {"name": "🏥 الصحة", "fields": ["patient_name", "patient_age", "village", "symptoms", "disease_type"]},
        "education": {"name": "📚 التعليم", "fields": ["school_name", "village", "students_count", "teachers_count", "needs"]},
        "livestock": {"name": "🐄 الثروة الحيوانية", "fields": ["farmer_name", "farmer_phone", "village", "animal_type", "animal_count"]},
        "water": {"name": "💧 المياه", "fields": ["village", "water_source", "water_functional", "distance_km"]},
        "energy": {"name": "⚡ الطاقة", "fields": ["village", "has_electricity", "main_energy_source"]}
    }
...
sector_names = [sectors_data[s]["name"] for s in sectors_list]
...
for i, field_name in enumerate(fields):
    with col1 if i % 2 == 0 else col2:
        label = t(field_name) if field_name in translations[st.session_state.language] else field_name.replace("_", " ").title()
